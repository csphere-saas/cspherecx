...
IMPORTANT: Return ONLY valid JSON with this exact structure. Do not include any explanations, notes, or text outside the JSON object.
Double-check that your JSON is properly formatted with:
- All keys in double quotes
- No trailing commas
- Properly closed brackets and braces
- All strings properly escaped

CRITICAL: Return ONLY the JSON object. Do not add any text before or after it.
...
{{
    ...
    def _parse_analysis_response(self, response_text: str, 
        ...
        # Debug logging
        logger.debug(f"Raw Gemini response length: {len(response_text)}")
        ...
        try:
            ...
            try:
                ...
                logger.warning(f"Initial JSON parse failed: {str(json_error)}. Attempting to fix...")
            ...
            # Debug the specific error area
            self._debug_json_response(response_text, str(e))
    ...
    def _fix_common_json_issues(self, json_str: str) -> str:
        ...
        try:
            # Try to parse line by line and fix issues
            ...
            # If we can't fix it, return the original and let it fail
            return json_str
    
    def _debug_json_response(self, response_text: str, error_msg: str):
        """Debug method to help identify JSON parsing issues"""
        try:
            logger.debug("=== DEBUG JSON RESPONSE ===")
            logger.debug(f"Error: {error_msg}")
            
            # Find line numbers if mentioned in error
            line_match = re.search(r'line (\d+)', error_msg)
            if line_match:
                line_num = int(line_match.group(1))
                lines = response_text.split('\n')
                if len(lines) >= line_num:
                    logger.debug(f"Problematic line {line_num}: {lines[line_num-1]}")
            
            # Find char position if mentioned in error
            char_match = re.search(r'char (\d+)', error_msg)
            if char_match:
                char_pos = int(char_match.group(1))
                start = max(0, char_pos - 50)
                end = min(len(response_text), char_pos + 50)
                logger.debug(f"Around char {char_pos}: ...{response_text[start:end]}...")
